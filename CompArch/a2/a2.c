...
int main() {
  ...
  do {
    ...
    // Assume float 1 is the larger exponent.
    emulated = add_floating_point(float_1, float_2);

    // See if float2 is the larger one, if so recalculate.
    if(float_1.f_bits.exponent < float_2.f_bits.exponent) {
      emulated = add_floating_point(float_2, float_1);
    }

    // Save the emulated float value into the float_32 container.
 ...
 *  I use an if statement in main to make sure that first_int is the larger
 *  of the two, or they are equal.
 */
float add_floating_point(float_32 first_int, float_32 second_int) {
