...
Status generic_vector_push_back(GENERIC_VECTOR hVector, void* item)
{
	...
	if(vector != NULL)
	{
		...
		//insert the element at position size...
		vector->functions.assignment(&(vector->data[vector->size]), item);  
...
GENERIC_VECTOR vector_of_vector_init_default(GENERIC_VECTOR hVectofVect)
{
	...
	for(i = 0; i <  29; i++)
	{
	// 	Make a vect of MYSTRINGS
		hVectofStr = make_vector_of_mystring();
		
	//	Store the vect of MYSTRING in the vector of vector
		test = generic_vector_push_back(hVectofVect, hVectofStr);
		
	//	See if it was a success or failure.
		if(test == FAILURE)
		{
		//	It failed. Quit with an error.
			printf("Error - failed to push VectofStr into VectofVect!\n");
			return NULL;
		}
...
void myvector_assignment(void** left, void* right)
{
	...
	if(real_left == NULL)
	{
		...
		*left = real_left = (Generic_vector*)make_vector_of_mystring();
	}
...
{
//	For a vector of vector, we must destroy each vect of mystring,
//	using generic_vector_destroy to do so.
