...
Node* Balance_Left(Node *head, Boolean *taller)
{
	Node* left = head -> left;				// Left subtree of root
	Node* right;							// Right subtree of root
	
	switch (left -> bf)
	{
		case RH:							// Double roation right
			right = left -> right;			// (Left Right Case)
			...
			right -> bf = EH;				
			head -> left = RotateLeft(right);
			head = RotateRight(head);
			break;
		
		case EH:							// No need to rotate!
			printf("Error: the tree is already balanced!\n");
			break;
...
Node* Balance_Right(Node *head, Boolean *taller)
{
	...
	switch (right -> bf)
	{
		case RH:								// Single rotation left
			head -> bf = right -> bf = EH;		// (Right Right Case)
		...
		case LH:								// Double roation left
			left = right -> left;				// (Right Left Case)
