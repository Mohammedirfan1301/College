...
Node*           Remove(Node *head, int item, Boolean *shorter);
...
Node* Remove(Node *head, int item, Boolean *shorter)
{
	...
	if(item < head->data)				// Go to the left side
	{
		head -> left = Remove(head -> left, item, shorter);
	}
	
	else if(item > head->data)			// Go to the right side
	{
		head -> right = Remove(head -> right, item, shorter);
	}
	
	else if(item == head->data)			// Found it, so delete it!
	{									// Couple of cases to run through
		// NO sub children
		...
		if(head->right != NULL && head->right != NULL)
		{
			...
			// Now copy temp into head
			head -> data = temp -> data;
			
			// Now delete the inorder successor
			head -> right = Remove(head -> right, temp -> data, shorter);
