...
Sierpinski::Sierpinski(int depth, int side) : _depth(depth) {
  ...
  float height = .5 * sqrt(3.0) * static_cast<float>(side);
...
Sierpinski::~Sierpinski() {
  ...
  // After we clear the allocated objects, we don't have to worry about
  // stuff allocated on the stack since that will be handled automagically.
}
