...
main () 
{
    ...
    {
        bool prime = true;             //bool can be true or false, so its set first to true (assuming the number is prime and must be proven to not be prime).
        
        for (b = 2; b * b <= a; b++)   //this does the numbers to test it with.
        {
            if (a % b == 0)            //if a % b = 0, the number cannot be prime.
            {
                prime = false;         //since it cannot be prime, it's false.
                break;                 //and this breaks it, as once its found to not be prime, there's no need to test further.
            }
        }   
        if(prime)                     //if it hasn't been proven to not be prime, then the number will be printed out below.
