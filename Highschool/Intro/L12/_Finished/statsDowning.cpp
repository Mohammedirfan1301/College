...
main()
{
    ...
    //key for the table
	 cout<<setiosflags(ios::right)<<setw(24)<<"Player's name"<<setw(20)<<"Batting Average"<<setw(23)<<"Number of Homeruns"<<endl;
    name = 0;
    playernum = 1;
    ...
    for(a = 0; a < 5; a++)
    {
		  cout<<"Player #"<<playernum<<": "<<setiosflags(ios::left)<<setw(13)<<names[name];  //prints the name of the player
                            //note that the left and setw(13) is to make the next setw line up correctly
        for(b = 0; b < 2; b++)
        {
            if(b == 0)
				cout<<setiosflags(ios::right)<<setw(15)<< setiosflags(ios::fixed) << setprecision (3)<<stats[a][b];
            if(b == 1)
            cout<<setiosflags(ios::right)<<setw(20)<< setprecision(0) << stats[a][b];
   ...
   cout<<"Thanks for using the STATS PROGRAM.";
