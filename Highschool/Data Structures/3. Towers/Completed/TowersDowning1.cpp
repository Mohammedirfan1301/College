...
//note that in a few functions, int const size is declared. this variable allows the changing of how many numbers (discs) to play the game with
//example, changing each 'size' to 3 would make the game use 3 discs and becomes much easier to test the program!
//the default (as the program is now) is eight.
...
main ()
{
	...
	print(peg1, peg2, peg3);	//prints the arrays
	...
	{
		print(peg1, peg2, peg3);	//print the arrays to see how the player won.
		x = 2;	//so make the game end by making x 2 and print the below winner message
		cout<<"\nYOU HAVE WON DA GAME! Congratz! \n";
...
void initializeboard(int arr1[], int arr2[], int arr3[])
{
	...
	const int size = 8;	//how many discs (#s) to be used
...
int wincheck(int arr3[])
{
	...
	if(b == (size + 1)) //if b makes it all the way to eight, then each number has been found to be stored in peg3. the only way for this to happen
...
int move(int stack1[], int stack2[])
{
    ...
    if(error != -1 && error != -2 && error != -3)
    {
		...
		if(d == 0)
			stack2[size] = top1;	//push the other number to the second array.
		else
			stack2[d-1] = top1;
	}
