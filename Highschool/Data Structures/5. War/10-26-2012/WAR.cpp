...
void deal(char deck1[52][2], char deck2[52][2])
{
	...
	int a, b = 0, x = 1, z = 0;
	int num = 1;				//number of cards pushed to shuffle
	int norun = 0;
	char shuffle[51][2] = {0};	//contains a shuffled card deck after the following code is excuted
	int nums[51] = {0};		//holds all the numbers that have been shuffled to the shuffle deck
	...
	do{
		...
		for(a = 0; a < 10; a++)	//check to make sure it isn't in the used number array
			if(nums[a] == z)
				norun = -1;
