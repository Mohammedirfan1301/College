...
void printword(int g, int wordlen, char guess[30], char input[30], char real[30]);			//Prints the word as it is guessed so far
void printman(int tries);																//Prints Hangman as he appears thus far
int compare(int wordlen, int tries, int g, char input[30], char guess[30], char real[30]);	//Compares each guess to see if the word has been guessed correctly yet.
...
int PlayRound(node **head)
{
	...
	g = 0, tries = 0, wordlen = 0, result = 0;
	...
	while(1){
		...
		cout<<"WORD = "<<real;
		cout<<"\n\nThe word 'is': ";
    	printword(g, wordlen, guess, input, real);
		cout<<"\n\nGuess a letter. -> ";
		cin>>input[g];
...
void printword(int g, int wordlen, char guess[30], char input[30], char real[30])			//Prints the word as it has been guessed
{
    int a = 0;
    cout<<guess<<input<<real;
...
int compare(int wordlen, int tries, int g, char input[30], char guess[30], char real[30])
{
    int a, corlet = 0;
    ...
    if(corlet == 0 && tries < 6)			//The guessed letter was NOT in the word - thus increase tries.
    {
		tries++;
		return tries;
	}
	
	if(corlet > 0 && strncmp(guess, real, wordlen) != 0)			//The letter guessed was in the word - nothing happens!
    	return tries;												//Also the word has not completely been guessed - hence not equal to 0
    
    if(corlet == 0 && tries == 6)			//You lost! :(
		return -2;							//Because the guessed letter wasn't in the word and you've run out of tries! (only 6 tries!)
    
    if(strncmp(guess, real, wordlen) == 0)	 //You won! :)
        return -1; 							//Since none of the above ifs caught it, and the word was guessed correctly as comparing the guessed word
}											//to the real word equals 0!
