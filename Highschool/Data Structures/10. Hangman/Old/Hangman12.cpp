...
int PlayRound(node **head)
{
	...
	string theword;									//String to hold the word
	theword = ChooseWord(head);						//Generates a random word from the ChooseWord function
	
	strcpy(real, theword.c_str());					//Copy the word to a char array
	wordlen = strlen(real);							//Find the length of the word
	
	g = 0, corlet = 0, tries = 0, wordlen = 0, result = 0;		//Make sure everything is zero!!!
	...
	while(1)
	{
		...
		cout<<"WORD = "<<real[0];			//REMOVE - THIS IS FOR DEBUGGING!
    	...
    	if(strncmp(guess, real, wordlen) == 0)	 //You won if this happened. Aka the guessed word equaled the real word.
		{
			result = 1;    		//You won! :)
			break;		
		}
