...
int compare(int wordlen, int z, char input[30], char guess[30], char real[30]);			//Compares each guess to see if the word has been guessed correctly yet.
...
int PlayRound(node **head)
{
	...
	int a = 0, wordlen = 0, x = 1, tries = 0, z = 0;
	int result = 0;
	string theword;
	...
	do{
		...
		cout<<"\nGuess a letter. -> ";
		cin>>input[z];
    	tries = compare(wordlen, z, input, guess, real);
...
int compare(int wordlen, int tries, int z, char input[30], char guess[30], char real[30])
{
    int a, corlet = 0;
    ...
    if(corlet == 0 && tries == 7)
		return -2;	//Uh-oh, you lost! :(
       
    if(strncmp(guess, real, wordlen) == 0)
        return -1;   //You won!
}
