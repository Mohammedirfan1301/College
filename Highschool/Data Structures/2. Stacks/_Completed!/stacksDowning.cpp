#include <iostream>	//i declared a lot of headers but each one is used at some point in the program.
#include <fstream>
#include <sstream>
#include <string>
#include <conio.h>
...
void manual(int stack[])
{
    ...
    switch(comd)
    {
        case 1:
            ...
            cin>>temp;
            cout<<"\n";
            code = push(stack, temp);	//code is equal to whatever the push function returns - which is either an error code (-1) or zero for succes
            if(code == 0)	//success!
        ...
        case 4:
            cout<<"\n*** Print Function. ****\n";
            code = print(stack);	//if everything works, the print function prints. it only returns -1 if there is an error ie the stack is empty.
            if(code == -1)	//error!
        ...
        case 5:
            ...
            for(a = 0; a < 10; a++)	//run 10 times in case there are 10 values in the array.
            	pop(stack);		//makes sure the array is cleared for the next run through by popping any values that are still in the array.
            break;
