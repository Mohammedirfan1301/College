...
void file(int stack[])
{
    ...
    string command;     //string to hold each line in the file
   	char carg[20] = {0};	//char array that can hold the line.
   	int num = 0;		//final num to push to the functions
    //int cmdnum[10] = {0};	//holds the int values from the second string
    int a, b, x = 1;	//a/b are for loops/temp vars. x is for the loop
    ...
    do{
        ...
        strcpy(carg, command.c_str());
        
        if(strncmp(command.c_str(), "PUSH", 4) == 0)
        {
			num = atoi(carg);
			cout<<num<<endl;
			//ascii value of P + U + S + H = 320
			//num2push = num2push - 320;
			//cout<<num2push;
		}
