...
int main()
{
    ...
    char recipt[100][10] = {0};	//hold array for the barcodes.
    ...
    {
        if(strcmp(barcode[b], barcode[b+1]) == 0)   //checks to see if the barcode in barcode[a] is the same size as barcode[a+1]
            b++;    //if so it increases a
        else
            numofit = b;        //if not, this means barcode[a+1] is not 8 characters long and thus not a barcode
    ...
    while(e == 0)	//run until the end of all the transactions
    {
		...
		while(t == 0)	//run until the end of this current transaction
		{
            ...
            {
                if(strncmp(recipt[a], barcode[b], 8) == 0)  //if there is a match, do this
                {
					x = 1;
					break;
                }
