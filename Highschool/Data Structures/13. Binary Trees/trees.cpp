...
struct node			//A structure, which contains all the data (int + pointer) for each element (node) in the linked list
{
	...
	node *left;		//This will point to the next node.
	node *right;	//This will point to the last node.
};

//REQUIRED FUNCTIONS
int Read(node *&headl, node *&headt)
int Add(int a, node *&head, node *&tail);	//Function to add numbers to the list
int Print(node *head);						//Test function - used during beta testing to make sure the sorting functions ACTUALLY sorted the data.
int Destroy(node *&head);					//Function to DESTROY the list at the end of the program
...
int Destroy(node *&head)		//Function that deletes the list from memory
{	
