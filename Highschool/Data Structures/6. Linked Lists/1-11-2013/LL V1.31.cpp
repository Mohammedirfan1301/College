...
void Add(int a, node* head)
{	
	//First create a new node and store data
	node* NN = new node;
	NN -> data = a;
	NN -> next = NULL;
	
	//Now create a temporary pointer.
	node *temp = head;
	
	//If the list is empty, its the first node in the list!
	if(temp == NULL)
	{
		head = NN;
	}
	//Else it isn't empty and thus add at the end of the list!
	else				
	{

		while(temp -> next != NULL)	                //find the last node
			temp = temp->next;						//and store it in the tmp pointer.
		
		//Now that I have the last node in the list I can point it to the new node!
		temp -> next = NN;
	}
