...
{
	node *curr, *last, *cont, *temp, *min;	//Temp pointers. Curr/Last are head/tail. Cont is where to continue the list from
	int hold;
	last = *tail;
...
{											
	...
	{
		piv = part(head, tail);			//Split the list into two!
		QuickSort(head, piv->prev);	//Sort the right half
		QuickSort(piv->next, tail);	//Sort the left half
	}
...
{
	int pivot = head -> data;				//I picked the pivot value as the first int in the list
    node *left = head, *right = tail;		//Head is basically left, tail is basically right!
    int temp;
 
    while(head -> data < tail -> data)	//Keep moving until head's data is less than tail's data
    {
        while(head -> data <= pivot)	//Move right
            head = head -> next;
        while(tail -> data > pivot)		//Move left
            tail = tail -> prev;
		if (head -> data < tail -> data)//Looks like we need to do a swap here!
        {
                   temp = head -> data;	//Hold head's data
          head -> data  = tail -> data;	//Head's data = tail's data
           tail -> data = temp;			//Tail's data = the held data from above ^
        }
    }
    left -> data = tail -> data;	//Swap the pivot at the end
    tail -> data = pivot;
    
	return tail;	//Return the "Right" or Tail
}
