...
//struct node *QuickSort(node *head)		//Note that left = head, right = tail
{											
	//FUCK BROKE SOMETHING FIX IT SEE THE TOP CODE SINCE IT ACUALLY WORKS!!!!
	
	//Return if the list is NULL
	if(head == NULL)
		return head;	//Returning head because I must return something!
	
	//Now I defined several pointers. Less/Great hold the less/great lists. Pivot is just head, this is the
	//value to compare to. curr is the current node we are comparing, and end is the last node in the list
	node *less = NULL, *great = NULL, *pivot = head, *next, *end, *curr = head -> next;
	...
	while(curr != NULL)
    {
        ...
        {
            curr -> next = less;			//Then send the node to the 'less than' list
                    less = curr;
        }
        else								
        {
    ...
    if(less != NULL)
    {
        ...
        while(end -> next != NULL)	//Find the last node in the list (ends when the next val is NULL)
         ...
         end -> next = head;		//Combine's the lists
        head -> next = great;
