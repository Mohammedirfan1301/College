...
void QuickSort(node *head)		//Note that left = head, right = tail
{											
	node *root, *less = NULL, *equal, *great = NULL;
	int piv = head -> data;	//piv value is the first value in the list
	
	//Sort the list into two lists - less than the piv and greater than the piv
	for(root = head; root -> next != NULL; root = root -> next)
	{
		if(root -> data < piv)
			QAdd(root->data, less);
		if(root -> data > piv)
			QAdd(root->data, great);
		if(root -> data == piv)
			QAdd(root->data, equal);
	}
	
	//Now that the list is split into two lists - call quicksort again!
	QuickSort(less);
	QuickSort(great);
	
	//Now combine all the lists into one!
...
void QAdd(int a, node *&head)
{
	...
	if(!curr)							//If temp threw a NULL, then it seems memory allocation failed!
		return;							//Therefore return to show memory allocation failed error to the user.
	...
	if(head == NULL)					//IF head is NULL, then the list is empty!
		...
		curr -> prev = NULL;			//Make curr's prev point to befo.
