...
//REQUIRED Functions for the program
void ReadData(struct node *head, struct node *tail);//Handles file related stuff (takes in all the data)
int SortCheck(struct node *head, struct node *tail);//Checks to make sure the sort function worked properly
void SortReport(double times[]);					//Reports how long each sorting element took
void Destroy(struct node *head);					//Function to DESTROY the list at the end of the program

void BubbleSort(struct node *head,struct node *tail);
void SelectionSort(struct node *head,struct node *tail);	//Sorting functions
void MergeSort(struct node *head,struct node *tail);
void QuickSort(struct node *head, struct node *tail);

//Other functions used
void WhichSort(int a);
int Add(int a, struct node *head, struct node *tail);	//Function to add numbers to the list
int Remove(int a, struct node *head, struct node *tail);//Removes from the list
void Print(struct node *head, struct node *tail);		//Test function - used during beta testing to make sure the sorting functions ACTUALLY sorted the data.
...
int main()	//ZE MAIN
{
	...
	do{
		struct node *head = NULL; 		//head of the linked list - points to the start of the list. Set to NULL as there aren't any elements in the list (yet).
		struct node *tail = NULL;		//tail of the linked list - points from the end of the list back toward the front.
...
void QuickSort(struct node *head, struct node *tail)
{
	node *less = NULL, *great = NULL;	//Less = values less than the pivot, great = values greater than than the pivot
	int a, piv;									
	
	if(head == NULL)								
	{														//If head is NULL there is no list to sort!
		cout<<"\nError! Cannot sort an empty list! \n";		//ERROR!1!1
		return;												//Stop the function from going on
	}
	...
	//First split the list into to two - less than and greater than lists
	if(head != NULL && head != tail && tail != head -> next)
	{
	...
	//Then call quicksort again for each list
	//QuickSort(&less, &tail);
	//QuickSort(&great, &tail);
}

//-----------------------------------------------------------------------------------------------------------------------------
//These functions handle the basic doubly linked operations. Ie ReadData, Adding, Deleting the list, etc.

void ReadData(struct node *head, struct node *tail)
{
...
int Add(int a, struct node *head, struct node *tail)	//Adds data to the linked list
{	
...
int Remove(int a, struct node *head, struct node *tail)
{
...
void Destroy(struct node *head)		//Function that deletes the list from memory
{	
