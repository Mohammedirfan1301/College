...
void SortCheck(node *head);					//Checks to make sure the sort function worked properly
...
//===========================================================================================================================================================
	...
	//BUBBLE SORT
		...
		SortCheck(head);		//This checks to see if the BubbleSort function sorted the list correctly
		Print(head, tail);		//Beta testing purposes
...
//THIS DOESN'T WORK AND I DON'T KNOW WHY!!! FIX ME!!!
//void SortCheck(node *head)	//Checks to make sure the sort function worked properly
{
	...
	for(node *curr = head; curr -> next != NULL; curr = curr -> next)	//Run through the entire list
	{
		if(curr -> data <= curr -> next -> data)		//If this current node is less than the next node
			//DO NOTHING ITS FINE						//Then the sort function worked and move forward 1 node
		if(curr -> data > curr -> next -> data)			//Otherwise something went wrong
		{
			cout<<"ERROR! LIST NOT SORTED! RESULTS ARE INVALID!!! \n";
			return;										//So exit and return to signal the sort was not complete!
	...
	return;		//So return to signal success!
}
