...
struct node *Merge(node *left, node *right); //Not required but I used this function to merge all the sublists together
...
//Merge Sort! Quick but really annoying.
/*
PROCESS OF MERGE SORT:
	1. Split the list into two sublists
	2. Use recussion to split these sublists into smaller and smaller lists
		until each list consists of 1 or 2 elements each
	3. Merge these lists together and return the final sorted list
*/
struct node *MergeSort(node *head)	//head in this case could either be *head (original list), left or right sublists
{
	node *left, *right;				//The two lists
	
	if((head == NULL) || (head -> next == NULL))	//If head or head's next are NULL, then list is sorted	
		return head;								//So return head
	
	//Now split the list into two sub lists
	 left = head;			//Hold head in the left list
	right = head -> next;	//Right will start out one ahead of left
	
	while(right && right -> next) 	 	//Find the halfway point, by running one pointer at twice the speed as the other
	{
  		 head = head -> next;			//Head moves forward ONE node for each pass
  		right = head -> next -> next;	//Right moves forward TWO nodes for each pass
 	}
 	       right = head -> next;	//Right now becomes the head of it's own list
 	head -> next = NULL;			//Left was originally equal to head, so now it has its own list that ends at the halfway point
									//As at this point head is in the middle of the list, so by making it equal to NULL we've broken the left list in the middle
	
	//Call MergeSort to split each list until each list has 1 node
	 left = MergeSort(left);			//Split the left side
	right = MergeSort(right);		//Now split the right side

	return Merge(left, right);		//Return the final sorted list! Yay!
}

struct node *Merge(node *left, node *right)
{
	node *end = NULL, *root = NULL;
	//Now merge the lists together
	if(left == NULL) 			//left is empty no merging needed just return right
		return right; 	
	else if(right == NULL) 		//right is empty so return left
		return left; 	
	else						//Otherwise merge the left & right lists together!
	{	//First check the first nodes
		if (left -> data <= right -> data) 	//If left's data is less than or equal to right's data, then
		{									//add left's data to the new sorted list
			root = left;					//root is the 'finished' list, so make it point to left
			left = left -> next;			//Move left forward one node
			end = root;						//end is the end of the list - make it equal to root for now
		}
		else 								//This is the reverse of the above check - left's data is actually greater than right's
		{		
			root = right;				
			right = right -> next;			//So just do the reverse - make root point to right, move right forward one, etc
			end = root;
		}
		//Now run through the lists
		while(left != NULL && right != NULL) //Run until either list is equal to NULL
		{
			if(left -> data <= right -> data)//Same check as the above code, except this one runs for the rest of the list
			{								 //Also, root isn't changed as it points to the rest of the list. Instead:
				end -> next = left;			 //Make end point to left
				        end = end -> next;   //^
				       left = left -> next;	//Move left forward one node
			}
			else							//Other wise left data > right data, so add right to the finished list!
			{
				end -> next = right;		
				        end = end -> next;	//Same as above - end points to right, right moves forward one node
				      right = right -> next;
			}
		} 									//END OF THE WHILE AT THIS POINT
		//This finishes up the sorting process by adding which ever list equaled NULL to the end of the sorted list

		if(left == NULL)			//If left was the first to equal NULL, then right has one node left
			end -> next = right;	//So make end point to right (and right's next equals NULL, becoming the end of the sorted list)
		else 						//Reverse is true - right was the first to equal NULL
			end -> next = left;		//So just make end point to left
		return root;	//Return the head of the final, sorted list! FINALLY!
	}	
}
