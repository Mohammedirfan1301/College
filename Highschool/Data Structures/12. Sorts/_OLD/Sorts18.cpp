...
{
	node *root = *head, *end = *tail;
	int pivot = root -> data;				//I picked the pivot value as the first int in the list
    node *left = *head, *right = *tail;		//Head is basically left, tail is basically right!
    int temp;
 
    while (root -> data < root -> data)	//Keep moving until head's data is less than tail's data
    {
        while(root -> data <= pivot)	//Move right
            root = root -> next;
        while(end -> data > pivot)		//Move left
            end = end -> prev;
		if (root -> data < root -> data)//Looks like we need to do a swap here!
        {
                   temp = root -> data;	//Hold head's data
          root -> data  = root -> data;	//Head's data = tail's data
            end -> data = temp;			//Tail's data = the held data from above ^
        }
    }
    left -> data = end -> data;	//Swap the pivot at the end
    end -> data = pivot;
    
    *head = root;	//Make sure Head/Tail are updated
    *tail = end;
    
	return end;	//Return the "Right" or Tail
}
