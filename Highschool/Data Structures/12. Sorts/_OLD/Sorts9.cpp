...
void SelectionSort(node **head, node **tail)	
{
	...
	{
		min = curr;		//Make min equal to the current data position (to compare against the other numbers)

		for(cont = curr -> next; cont -> next != NULL; cont = cont -> next)	//Start comparing from the next node on to the end
		{
			if(cont -> data < min)	//If it finds a value that is less then the current min
			{
				min = cont -> data;	//Then this becomes the new minimum!
				temp = cont;		//Note the node for future reference
			}
		}
		
		if(min != curr -> data)	//If min is different then what it originally was (ie a swap should take place)
		{
			        hold = curr -> data;
			curr -> data = min;
			temp -> data = hold;
