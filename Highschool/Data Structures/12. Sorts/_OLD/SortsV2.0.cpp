...
//Merge Sort! Quick but really annoyingly.
struct node *MergeSort(node *head)	
{
	...
	node *left = NULL, *right = NULL, *hold;
	int x, mid = 0, tot = 0;
	
	hold = head;				//Hold head so as to not change it
	while(hold != NULL)			//Run til the end of the list
	{
		hold = hold -> next;	//Move forward 1 node
		tot++;					//Tot will, at the end of this while loop, equal the total # of nodes in the list
	}
	mid = tot/2;				//Mid = middle of the list or total # of nodes / 2
	
	//Divide the list into two equal parts
	for(x = 0; x > mid; x++)			//Run from 0 to the middle of the list (if length of list = 100, then mid = 50)
	{									
				hold = head -> next;	//Hold head's next
		head -> next = left;			//link head's next to left
				left = head;			//left now points to head
				head = hold;			//head now equals the hold from above
	}
	for(x = mid; x <= mid && head != NULL; x++)
	{
				hold = head -> next;	//Same as above, but for the right side of the list
		head -> next = right;
			   right = head;
				head = hold;
	}
	...
	//Merge the lists
	head = Merge(left, right);	//Merge the two lists into one, or "head"
	return head;				//Return the final sorted list! Yay!
}

struct node *Merge(node *left, node *right)	//Helper function to MergeSort - merges the sublists together
{
	node *final = NULL;	//Will hold the completed list at the end, to be returned to MergeSort
	node *hleft, *hright;
	int piv = left -> data;
	
	//Adapted from HERE!
	//http://www.cplusplus.com/forum/beginner/21319/
	
	hleft = left;
	final = left;
	
	while(hleft -> next != NULL)
		hleft = hleft -> next;
	...
	return final;	//Return the completed list - WE ARE DONE FINALLY!
}
