...
int main()
{
	...
	do{
		...
		BubbleSort(&head, &tail);	//BubbleSort!
...
int BubbleSort(node **head, node **tail)
{
	...
	do{
		...
		do{    
			...
			curr -> next -> data = temp -> data;			//Make the previous node equal to the next node
				    		swap = 1;						//Flag this pass as having a swap
			}
			curr = curr -> next;		//Move temp up one node	
		}while(curr -> next != *temp);	//If temp equals tail, it has hit the end of the list!
