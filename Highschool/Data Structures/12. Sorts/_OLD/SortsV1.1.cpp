...
void QuickSort(node *right, node *left);

//Other Functions
node * part(node *head, node *tail);		//USED IN QUICKSORT!
...
//===========================================================================================================================================================
	...
	//BUBBLE SORT
		...
		//ret = SortCheck(head);		//This checks to see if the BubbleSort function sorted the list correctly
...
void BubbleSort(node *head, node *tail)	//Bubblesort! Simple yet slooooow!
{
...
void SelectionSort(node *head, node *tail)	//Selection Sort! More complex but slightly quicker then Bubble Sort!
{
...
void MergeSort(node *head, node *tail)	//Merge Sort! Quick but really annoyingly complex.
{
	...
	if(head -> next == NULL)//There's no need to sort - there's only one element!
		return;				//So stop this function from continuing.
	
}
											//Quick Sort - Quick and *only* slightly annoying.
void QuickSort(node *right, node *left)		//Note that right = head and that left = tail
{											//Although that it is not necessary true for multiple run throughs (recurrision)
	node *piv;								//used to hold the piv node		
	
	if(right -> data < left -> data)		//If right's data is less then left's data
	{
		piv = part(right, left);			//Split the list into two!
		QuickSort(right, piv->prev);		//Sort the right half
		QuickSort(piv->next, left);			//Sort the left half
	}
	
}

node * part(node *right, node *left)
{
	int pivot = right -> data;				//I picked the pivot value as the first int in the list
    node *l = left, *r = right;				//Stored for later use
    int hold;								//A hold variable
 
    while(right -> data < left -> data)		//Keep moving until head's data is less than tail's data
    {
        while(right -> data <= pivot)		//Move right
            right = right -> next;
        while(left -> data > pivot)			//Move left
            left = left -> prev;
		if (right -> data < left -> data)	//Looks like we need to do a swap here!
        {
                   hold = right -> data;	//Hold head's data
         right -> data  = left -> data;		//Head's data = tail's data
           left -> data = hold;				//Tail's data = the held data from above ^
        }
    }
    l -> data = right -> data;				//Swap the pivot at the end
    right -> data = pivot;
    
	return left;							//Return the "Right" or Tail
}
...
int SortCheck(node *head)	//Checks to make sure the sort function worked properly
{
	node *curr;				//Temporary pointer
	curr = head;			//Start off with curr equaling head
	
	if(head == NULL)		//If head is still NULL, then clearly something went wrong with the sorting.
		return 1;			//OR something went wrong with the ReadData function!
	
	do{
		if(curr -> data <= curr -> next -> data)			//If this current node is less than the next node
			curr = curr -> next;						//Then the sort function worked and move forward 1 node
		if(curr -> data > curr -> next -> data)	//Otherwise something went wrong
			return 1;									//So exit and return 1 to signal the sort was not complete!
	}while(curr != NULL);	//Run until the very last node
							//Everything was correctly sorted
	return 0;				//So return 0 to signal success!
}
