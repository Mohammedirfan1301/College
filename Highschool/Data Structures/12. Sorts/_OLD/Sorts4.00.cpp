...
PROCESS OF MERGE SORT:
	...
	2. Use recussion to split these sublists into small and small lists
		until each list consists of 1 or 2 elements each
	3. Merge these lists together
...
struct node *MergeSort(node *head)	
{
	...
	//This works by moving right ahead by two each pass, and setting head equal to NULL at the middle point
	while(right && right -> next) 	//So it ends up with 
	{
  		 head = head -> next;
  		right = head -> next -> next;
 	}
 	       right = head -> next;	//Break the list into two - right
 	head -> next = NULL;			//and left
	...
	node *nxt = NULL, *end = NULL, *finish = NULL;
	while (left || right)
    {
...
struct node *Merge(node *left, node *right)
{
	...
	if(left == NULL) 			//left is empty no merging needed
		return right; 	
	else if(right == NULL) 		//right is empty
		return left; 	
	else
	{
		if (left -> data < right -> data) 	//First check the first nodes
		{
			root = left;
			left = left -> next;
			end = root;						
		...
		//This while runs until EITHER left or right equals NULL
		while(left != NULL && right != NULL) 
		{
			if(left -> data < right -> data)
			{
				       ...
				       left = left -> next;		//Move left up one
			}
			else
			{
				end -> next = right;
		...
		//This finishes up the sorting process by adding which ever list equaled NULL 
		//to the end of the sorted list
		if(left == NULL)
			end -> next = right;
		else 
			end -> next = left;
		return root;	//Return the final
	}	
}
*/
