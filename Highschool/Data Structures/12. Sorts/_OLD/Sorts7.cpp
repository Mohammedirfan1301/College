...
void SortReport(double times[])	//Reports how long each sorting element took
{
	cout<<"Which sorting method was the quickest? Well... \n";
	
	double hold = 0;
	double timesO[3];
	int swap = 0, a = 0;
	
	for(a = 0; a < 4; a++)		//Creates a duplicate list of the times (for comparision after sorting them)
		timesO[a] = times[a];
	
	do{
		if(times[a] > times[a+1]){		//Basic bubble sort of the times
				  hold = times[a];
		      times[a] = times[a+1];
			times[a+1] = hold;
				  swap = 1;
	...
	for(a = 0; a < 4; a++){		//1st Fastest
		if(timesO[0] == times[a])
			cout<<"1st: Bubble Sort was the quickest with a time of "<<times[0]<<" seconds! \n";
		if(timesO[1] == times[a])
			cout<<"1st: Selection Sort was the quickest with a time of "<<times[1]<<" seconds! \n";
		if(timesO[2] == times[a])
			cout<<"1st: Merge Sort was the quickest with a time of "<<times[2]<<" seconds! \n";
		if(timesO[3] == times[a])
			cout<<"1st: Quick Sort was the quickest with a time of "<<times[3]<<" seconds! \n";
	}
	
	for(a = 0; a < 4; a++){		//2nd Fastest
		if(timesO[0] == times[a])
			cout<<"2nd: Bubble Sort was next with a time of "<<times[0]<<" seconds! \n";
		if(timesO[1] == times[a])
			cout<<"2nd: Selection Sort was next with a time of "<<times[1]<<" seconds! \n";
		if(timesO[2] == times[a])
			cout<<"2nd: Merge Sort was next with a time of "<<times[2]<<" seconds! \n";
		if(timesO[3] == times[a])
			cout<<"2nd: Quick Sort was next with a time of "<<times[3]<<" seconds! \n";
	}
