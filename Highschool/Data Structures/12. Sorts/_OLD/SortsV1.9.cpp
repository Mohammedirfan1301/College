...
struct node *MergeSort(node *head)	
{
	//If head or head -> next is NULL then its sorted (sublist of 0 or 1)
	if((head == NULL) || (head -> next == NULL))			
		return head;				//So return it!
	
	//Now split the list into two sub lists
	node *left, *right, *hold, *final = NULL;
	int x, mid = 0, tot = 0;
	...
	hold = NULL;
	//Merge the lists
	while(left || right)					//While either left or right is not equal to NULL
	{
		if(left == NULL)					//IF left equals NULL
		{
	   		 hold = right;					//then right becomes
	   		right = right -> next;	
		}
		else if(right == NULL)				//This is the opposite - right equals NULL
		{
	   		hold = left;					//so left
			left = left -> next;
		}		
	...
	return final;				//Return the final sorted list! Yay!
}
