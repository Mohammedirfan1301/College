...
{
	/*
	Make a second linked list
	
	Go through the original list and find the lowest number
	Add that number to the second list
	Repeat, until the first list is empty and the second list is the sorted list	
	*/
	
	node *root = NULL;		//head of the second linked list
	node *endd = NULL;		//tail of the second linked list
	node *curr, *last, *temp;	//Temp pointers
	int swap = 0, hold, code = 0;			//Equals 1 if a swap took place
	...
	do{
		curr = *head;	//Reset all the variables for the next pass
		last = *tail;
		temp = NULL;
		swap = 0;
		hold = 20000;	//Current "lowest" number (set high on purpose)
		do{	
			if(curr -> data <= curr -> next -> data){	//Is the number less then the next number?
				if(curr -> data < hold){				//If yes, is it less then the lowest current number?
					temp = curr;						//If this is also true, hold the number for future comparision
					hold = curr -> data;				//Also hold the node as well as the integer
					swap = 1;							//Flag that a swap has taken place
				}
			}
			curr = curr -> next;	//Move forward one
		}while(curr != last);		//Continue to the end of the list
		
		if(swap == 1)	
		{		//If a swap took place, first send the variable that is being swapped to the second linked list
				code = Add(hold, &root, &endd);	//Send it to the add function
				...
				//Then delete the node in the original linked list
				code = Remove(hold, head, tail); //Send it to the remove function
				
				if(code == 0)				//Everything was fine
					//cout<<"SUCCESS! \n";	//(Testing)
           		 if(code == -1){				//if ADD returns -1 bad things happened and therefore something went wrong
					cout<<"ERROR! Memory allocation failed! \n";	//aka memory allocation failed	
					break;
