...
/*	**************************************************************************************
	...
	*************************************************************************************/
	for(i = guesses; i > 0; i--)
	{
		...
		if(won == 0)
		{
			printf("Yes, there was a %c in the word! :]\n", letter);
		}
...
int CheckGuess(int word_size, char letter, GENERIC_VECTOR* word_list, MYSTRING* guessed_string)
{
	...
	// Destroy the current word list, as we will be replacing it soon!
	...
	temp_word_list = max_vector_node(hSet, &key);
	*word_list = temp_word_list;
	*guessed_string = key;
	
	/*		DEBUGGING INFORMATION.
	printf("This word size is: %d\n", generic_vector_size(*word_list));
	printf("Guessed string is: ");
	...
	*/
	...
	//	Other wise return 0 for failure.
	for(x = 0; x < word_size; x++)
	{
		if(mystring_get(*guessed_string, x) == '-')
		{
			// Found a dash, don't even bother contining the search!
			return -1;
		}
