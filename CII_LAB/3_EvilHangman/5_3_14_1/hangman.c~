               ...
               GENERIC_VECTOR* word_list, MYSTRING* guessed_string)
{
	...
	*newSet = my_set_init_default();
	...
	// If this is the first round, we must delete the original word_list!
	if(oldSet == NULL)
	{
		generic_vector_destroy(word_list);
	}
	// We we always grab the largest generic_vector from the tree however.
	*word_list = max_vector_node(*newSet, &key);
	...
	else if(oldSet != NULL) // There's an old associative array to destroy.
	{
