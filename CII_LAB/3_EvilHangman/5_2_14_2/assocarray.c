...
struct node
{
	/* 	Data (KEY) will act as the locater for each generic_vector. Each vector holds 
	separate lists of words, depending on which key is attached to that node.		*/
...
void     SearchTree(Node *head, MYSTRING key, Boolean *test);
int           FindSize(Node *temp, Node *numb);
Node*              Add(Node *next, MYSTRING key, MYSTRING word);
void			FindNode(Node* head, Boolean *test, MYSTRING key, MYSTRING word);
Node*           Remove(Node *next, MYSTRING key, Boolean *InList);
GENERIC_VECTOR 	max_vector_node(MY_SET hMY_SET, MYSTRING* key);
void 			FindMaxNode(Node* head, Node** max_node, int *size);
...
GENERIC_VECTOR max_vector_node(MY_SET hMY_SET, MYSTRING* key)
{
	My_set *pSet 	= (My_set*)hMY_SET;		// Get the head from the MY_SET object
	Node *head 		= pSet->root;
	Node *max_node 	= NULL;					// Used as a temp ptr
	int size = 0;
	...
	FindMaxNode(head, &max_node, &size);
	
	// See if something failed.
	if(size == 0 || max_node == NULL)
	{
		printf("Find max vector node failed!\n");
		return NULL;
	}
	
	/*	DEBUGGING SAVED ME FROM MANY HOURS OF FRUSTRATION...
	printf("I think the size of the max vector is: %d\n", size);
	printf("The node's size is like: %d\n", generic_vector_size(max_node->hVector));
	*/
...
void FindMaxNode(Node* head, Node** max_node, int *size)
{
	if(head != NULL)
	{
		...
		if(*size < (generic_vector_size(head->hVector)) )		
		{
			// Then save this node!
			*max_node = head;
			*size = generic_vector_size(head->hVector);
		}
		
		FindMaxNode(head->left, max_node, size);		// Search left
		FindMaxNode(head->right, max_node, size);		// Search right
	}
	
	// Return once we've searched the list and found the maximum vector
	return;
}
...
// Returns TRUE if key is in the set; exit on error
Boolean my_set_is_element(MY_SET hMY_SET, MYSTRING key)	
{
	...
	if(head == NULL)
	{
		printf("Error: can't find an key in an empty list!\n");
		return FALSE;
	}
			
	// Other wise run through the list until we find the key!
	if(head != NULL)
	{
		// Call Search tree to do this for us.
		SearchTree(head, key, &test);
	}
...
// Returns TRUE if an key is in the set. False if it is not.
void SearchTree(Node *head, MYSTRING key, Boolean *test)
{
	// Run through the list until we find the key!
	if(head != NULL)
	{
		if(mystring_compare(head->data, key) == 0)   // Uses MYSTRING_COMPARE
		{						// to see if these two MYSTRINGS are the same.
			*test = TRUE;
			return;
		}
		SearchTree(head->left, key, test);		// Search  left side
		SearchTree(head->right, key, test);	// Search right side
	}
	
	// If we get here, return FALSE - we didn't find the key.
	return;
}
...
// Adds key to the set if it is not already in it.
Boolean my_set_add(MY_SET hMY_SET, MYSTRING key, MYSTRING word)		
{
	...
	// First look and see if the KEY is already in the list
	FindNode(head, &test, key, word);
	
	// If we found that the key was already in the tree
	if(test == TRUE)
	{
		/* 	Return true, skipping the Add function since the key was already added & we
			just added the word to the vector	*/
		return TRUE;
	}
	
	// Other wise we can just see if there's room somewhere,
	// by looking left / right etc.
	head = Add(head, key, word);
	...
	// If head some how became NULL, something failed...
	return FALSE;
}
...
void FindNode(Node* head, Boolean *test, MYSTRING key, MYSTRING word)
{
	...
	if(head != NULL)
	{
		// If the key is the same as the data of this current node, then we found it!
		if(mystring_compare(head->data, key) == 0)
		{						
			// Just push the word back
			generic_vector_push_back(head->hVector, word);
			*test = TRUE; // Signal that we found the key & pushed the word to the vector.
			return;
		}
		
		// Go through the whole tree
		FindNode(head->left, test, key, word);		// Search  left side
		FindNode(head->right, test, key, word);		// Search right side
	}
...
Node* Add(Node *next, MYSTRING key, MYSTRING word)
{
	...
	if(next == NULL)
	{
		...
		code = mystring_concatenate_mystring(node->data, key);
		...
		// Create a generic_vector object for storing words of this key type
		...
		// Push the first word back to the generic_vector
		generic_vector_push_back(node->hVector, word);
	...
	// BUT I took care of that issue in the FindNode / MY_SET add functions.
	if(mystring_compare(next->data, key) == 0)
	{
	/*	We shouldn't get here if my code in MY_SET Add and FINDNODE work correctly...	*/
		printf("KEY was already in the set, why did we get here??\n");
		return next;
	}
	
	// Look to the left side
	if(mystring_compare(key, next->data) == -1)
	{
		next->left = Add(next->left, key, word);
	}
	
	// Look to the right side
	else if(mystring_compare(key, next->data) == 1)
	{
		next->right = Add(next->right, key, word);
	}
	...
	// Left Left		
	if (bf > 1 && ( (mystring_compare(key, next->left->data) ) == -1) )
	{
	    next = RotateRight(next);
	    return next;
	}

	// Right Right		mystring_compare(key, next->right->data)
	if (bf < -1 && ( (mystring_compare(key, next->right->data) ) == 1) )
	{
		next = RotateLeft(next);
	    return next;
	}

	// Left Right		( (mystring_compare(key, next->left->data) ) == 1)
	if (bf > 1 && ( (mystring_compare(key, next->left->data) ) == 1) )
	{
	...
	// Right Left	(mystring_compare(key, next->right->data) ) == 1)
	if (bf < -1 && ( (mystring_compare(key, next->right->data) ) == -1) )
	{
...
// Find and remove key from the set if it is present. Otherwise ignore.
Boolean my_set_remove(MY_SET hMY_SET, MYSTRING key)		
{
	My_set *pSet = (My_set*)hMY_SET;
	Node *head = pSet->root;
	Node *next = head;
	...
	// Other wise go through the list and try to find the key.
	next = Remove(next, key, &InSet);
...
Node* Remove(Node *next, MYSTRING key, Boolean *InSet)
{
	Node *temp;
	int bf;
	MyString_Status code;
	...
	// We found the end of a subtree, which means we didn't find the key!
	if(next == NULL)
	{
		printf("Error: ");
		mystring_output(key, stdout);
		printf(" is not in the list! Failed to remove it!\n");
		*InSet = FALSE;		// Signal to my_set_remove that we failed. :(
		return NULL;
	}	
	
	// If key is less than the current Node we are testing, go to the left subtree
	if(mystring_compare(key, next->data) == -1)				
	{
		next->left = Remove(next->left, key, InSet);
	}
	
	// Otherwise, go to the right subtree 
	else if(mystring_compare(key, next->data) == 1)			
	{
		next->right = Remove(next->right, key, InSet);
	}
	
	// WE FOUND THE key! Now we have a few cases to run through
	else if(mystring_compare(next->data, key) == 0)			
	{		
		// NO sub children
