...
/*	**************************************************************************************
	...
	*************************************************************************************/
	for(i = guesses; i > 0; i--)
	{
		...
		won = CheckGuess(word_size, letter, &hVectofMystring, &guess_string);
...
/*	
	...
	there is 1 possible word remaining and the guess_string is currently equal to that word.		
*/
int CheckGuess(int word_size, char letter, GENERIC_VECTOR* word_list, MYSTRING* guessed_string)
{
	MY_SET hSet;
	...
	word_list = (GENERIC_VECTOR*) max_vector_node(hSet, &guessed_string);
