...
GENERIC_VECTOR 	max_vector_node(MY_SET hMY_SET, int* max, MYSTRING* key);
...
int main(int argc, char * argv[])
{
	...
	printf("Associative Array test program - Version 5/2/14\n\n");
	...
	// Run a test from the dictionary file
	
	hVectofVect = vector_of_vector_dictionary();
	
	/*	
		In this test, I use basic dash keys (---) with no restrictions on what letters 
		are in the word. It basically sorts them based on size, putting
		each word into a certain sized key. Size 4 words go into ----, 5 into -----, etc.
		
		NOTE: actually, the vector_of_vector_dictionary file does the sorting of the word
		list by size! So really all I'm testing here is where the AVL tree can handle
		a ton of keys and words and remain balanced without crashing. I also use this
		to see if I can find the maximium word list from all the keys, so when I go to
		do the hangman.c code I can simply call a function and get the max vector and
		whether they won the game or not.
	*/
	
	for(i = 0; i < generic_vector_size(hVectofVect); i++)
	{
		temp = (GENERIC_VECTOR*)generic_vector_at(hVectofVect, i);
		
		if(temp == NULL)
		{
			printf("ERROR - generic_vector at failed!\n");
			exit(1);
		}
		...
		{
			printf("Error - failed to get vector of mystrings!\n");
			break;
		}
		
		key = mystring_init_default();
		
		// Make a blank key for use later.
		for(x = 0; x < i + 1; x++)
		{
			mystring_push(key, '-');
		} 
		
		for(x = 0; x < generic_vector_size(hVectofMystring); x++)
		{
			ptr = (MYSTRING*)generic_vector_at(hVectofMystring, x);
			...
			//	Put the MYSTRING object into the tree
			my_set_add(hSet, key, word);
		}
		
		// Make sure to destroy the key each round so we can make a new one!
	...
	// This was a basic test I ran - to see if I could add 3 words to the same key
/*
	...
	do{
		...
		switch(x)
		{
			...
			case 2:
				...
				hMax = (GENERIC_VECTOR) max_vector_node(hSet, &max, &key);
				if(hMax != NULL)
				{
					printf("The size of the maximum vector is: %d\n", generic_vector_size(hMax));
				}
				else
				{
