...
//	Allocate space for the opaque object
	pTemp = (MYSTRING) malloc(sizeof(struct _MYSTRING));	
...
/*	Got errors saying char* temp wasn't initialized, so I Googled the problem
	which suggested either calloc or using memset. I found memset to work, so
	I used the following code to initialize the char* array! */
	memset(temp, 0, MYSTRING_STARTING_CAPACITY);
...
int mystring_compare(MYSTRING source, MYSTRING test)
{
	...
	if((mystring_get(test, i)) == '\0')
	{
	//	Then we return 0 to indicate that source == test
		return 0;
	}

	// If we get here, source < test
	return -1;
}
...
//	Now determine whether we should increase hString->data's size.
	...
	//	Then there is no need to resize hString!
		for(a = hString_size; a < size_needed - 1; a++)
		{	
		//	c_string added to hString
			hString->data[a] = c_string[b];	
			b++;	// increase for accessing c_string
		}
		hString->size = size_needed - 1;	//	update hString's string size
...
//	c_string added to hString
	...
	hString->size = size_needed - 1;		// size updated
