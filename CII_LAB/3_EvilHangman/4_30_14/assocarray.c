...
Boolean my_set_add(MY_SET hMY_SET, MYSTRING item, MYSTRING word)		
{
	...
	// If head is NULL, SKIP the next few steps!
	if(head == NULL)
	{
		generic_vector_push_back(node->hVector, word);
		pSet->root = node;	// Updated pSet (hMY_SET) to include the new head.
		return TRUE;
	}
...
Node* Add(Node *next, Node *temp, Boolean *InSet, MYSTRING item, MYSTRING word)
{
	...
	/*
		seems to be an error with the same key repeating its self in several spots in the AVL tree???
	*/
	...
	// If we get here, we should be able to push the word back into the generic_vector!
